...
def make_judgement(agent,initial_code_response,query):
    ...
    """
    judgement=ask_gpt(make_judgement.format(agent_response=initial_code_response,request=query,
...
def run_llm_test(query,query_type='instruction'):
    ...
    """
    agent=ProxyAgent(json.load(open('/media/ssd/recovered_files/beaker-bio/context.json','r')),use_few_shot=True)
    trace=[]
    ...
    while attempts<3 and timer_to_gen_code<2:
        ...
        if code_response.startswith("ProxyAgent.SubmitCode: "):
            ...
            print(code_compiled,code_output_text)
            ...
            if code_compiled:
                ...
                if answer=='Yes':
                    print('Successful example, add example to json file or something with metadata, add code here')
                    print(code_response)
...
def run_llm_tests(tests:list,
    ...
    example_usage:
         ...
         {'test':'How can I a SIR model in mira?','query_type':'query','test_type':'submodule','tested_object':'mira.modeling.viz'}])
    ...
    if examples_json:
        print('strip out middle, convert to examples format dict')
        pass
